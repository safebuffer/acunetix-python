...
class Acunetix(object):
    ...
    def __headers(self):
        return (
            {"X-Auth": self.apikey, "content-type": "application/json"}
            if self.apikey
            else None
        )
    ...
    def __get_request(self, endpoint=None):
        return http_get_request(
            str("{}{}".format(self.host, endpoint if endpoint else "/")),
    ...
    def add_target(self, target=None):
        req = self.__post_request(
            endpoint="/api/v1/targets",
            data={"address": target, "description": "xxxx", "criticality": "10"},
        )
        return self.__json_return(req.text)
